...
pub fn generate_ec_group_file() {
    ...
    let ec0_str = ec0.to_bytes_str();
    println!("client priv_key:\n{}", ec0_str);
    file1.write(ec0_str.as_bytes());

    let mut file1 = File::create("ec_pub_key0").unwrap();
    let ec_pub0 = ec0.to_pub_handle();
    let ec_pub0_str = ec_pub0.to_bytes_str();
    println!("client pub_key:\n{}", ec_pub0_str);
    file1.write(ec_pub0_str.as_bytes());


    let mut file2 = File::create("ec_key1").unwrap();
    let ec1 = ecdsa::EcdsaHandle::new();
    let ec1_str = ec1.to_bytes_str();
    println!("server priv_key:\n{}", ec1_str);
    file2.write(ec1_str.as_bytes());

    let mut file2 = File::create("ec_pub_key1").unwrap();
    let ec_pub1 = ec1.to_pub_handle();
    let ec_pub1_str = ec_pub1.to_bytes_str();
    println!("server pub_key:\n{}", ec_pub1_str);
    file2.write(ec_pub1_str.as_bytes());
}
...
pub fn show_client_regist_priv_key() {
    let ec0_str = get_ec(0).to_bytes_str();
    println!("client priv_key:\n{}", ec0_str);

    let ec_pub1_str = get_ec_pub(1).to_bytes_str();
    println!("server pub_key:\n{}", ec_pub1_str);
}
...
pub fn test_generate_ecdsa_string_and_parse() {
    let test_privkey = comm::ca_manager::get_ec(0);
    let test_peer_pubkey = comm::ca_manager::get_ec_pub(1);

    let mut file = File::create("eckey_string").unwrap();
    let privkey_str = test_privkey.to_bytes_str();
    let pubkey_str = test_peer_pubkey.to_bytes_str();
    file.write(privkey_str.as_bytes());
    file.write(pubkey_str.as_bytes());

    let mut file = File::open("eckey_string").unwrap();
    let mut read_buf = [0u8; 256];
    let len = file.read(&mut read_buf).unwrap();
    let buf_cont = String::from_utf8(read_buf[..len].to_vec()).unwrap();
    println!("file read: {:?}", buf_cont);
    let test_eckey_copy = comm::ecdsa::EcdsaHandle::from_bytes_str(buf_cont);

    let msg = b"this is a ecdsa sign test";
    let sign_msg = test_privkey.sign_msg_with_bytes(msg);
    let veri_result = test_eckey_copy.veri_msg_with_bytes(msg, &sign_msg);
