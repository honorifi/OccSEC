...
clock_t hs_start, hs_end, msg_start, msg_end;
...
int do_one_connection(int argc, char** argv) {
    int ret;
    size_t len;
    ...
    hs_end = clock();
    hs_time += (double)(hs_end-hs_start)/CLOCKS_PER_SEC;
    ...
    msg_start = clock();
    len = sprintf((char*)buf, "Client Hello");
    ...
    msg_end = clock();
    msg_time += (double)(msg_end-msg_start)/CLOCKS_PER_SEC;
    msg_total += len;
...
int main(int argc, char** argv) {
    ...
    time_cost *= 1000; hs_time *= 1000; msg_time *= 1000;

    printf("\ntotal time cost: %f\n", time_cost);
    printf("average time:\n Handshake: %f\nmsg_trans: %f\n", 
    hs_time/repeat, msg_time*1024/msg_total);
}
